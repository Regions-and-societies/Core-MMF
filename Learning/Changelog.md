...
- **Fixed: placement rules were applied to tile searches, not only to the player's own settle checks (#37).** RimWorld and other mods look for site tiles (quest sites, mod-added sites) by testing thousands of candidate tiles with the game's "is this a valid settlement tile" check. Regions and Societies answered every one of those with a full placement evaluation and refused most of them on the player's supply rule, which could stall the game for a very long time when a mod ran such a search from a dialog, and starved quests of sites. The rules now apply only to checks a player is about to read (the settle button and the starting-site page), never to searches. The Vanilla Factions Expanded - Deserters comms-console freeze reported against 0.3.x is this bug; its mod-specific verification is tracked in the VFE compatibility patch.
...
- **New region partitioning.** Regions are drawn *inside* the terrain's natural sections — biome, mountains and coasts — then cut into even pieces, so borders follow the land rather than a grid, and region size varies with the terrain. Every region is now guaranteed to be a single connected landmass (a fix for provinces that used to span two separate islands).
- **Pluggable partition algorithm.** The partitioning method is now an extension point: choose it from a **World partition algorithm** dropdown in mod settings — *Contain then subdivide* (the new default) or *Anchor-Voronoi boxes* (the 0.2.x look) — and other mods can register their own. Each world is stamped with the algorithm and worldgen version it was generated with (shown as **Worldgen: v0.3.0** in the region panel), so loading an old save never re-cuts its map, and a regenerate reproduces it faithfully. See the [Developer's Guide](Developers_Guide#region-partition-algorithms-030).
- **Region inspection panel.** A three-tab region window — **Region** (shape mini-map, ownership, named features, resources, wildlife), **Population** (the seven demographic axes as bars and pies), and **Economy** (goods, housing, crises).
- **Faction character.** A faction's education, wealth and social standing now reflect what it *is* — raiders read as raiders, an imperial court as an elite — instead of every faction reading the same. Education is a five-level schooling ladder with defined meaning (skills, passions, economic role).
- **Settlement growth.** NPC settlements grow over time from in-game birthrates, converging on a tier target without exceeding it.
- **Worldgen performance.** Two world-generation steps that did not scale to the finer partition are out of this release: R&S settlement road-linking (its search could flood the whole landmass when two nearby bases were cut off by water; vanilla's own roads are unaffected) and outpost seeding at world generation (the seeding settings switch is inert this release). Both return reworked in 0.4.0. World generation now logs its total time, and placement checks that ask "is anything within N tiles?" stop searching at N instead of measuring the whole planet.
- **Residence layer.** Dwellings are modelled separately from raw population, with a rural-to-city occupancy gradient, and get their own overlay.
- **Population scale.** Settlement populations are three times larger (the cap multiplier default is 30, was 10: an industrial village caps at 30, a metropolis at 450), and a freshly generated world starts with its settlements at their comfortable size instead of a near-empty seed, so a settled region reads in the hundreds from day one and growth shows as crowding toward the tier maximum. A settings file still carrying the old default is lifted to the new one once; a value you set yourself is kept. The population sprawl is now the one population field: half of a settlement's people stay on its tile and the rest spread over the tiles its terrain-aware sprawl reaches (roads and rivers carry people further; hills, swamps and coasts thin them; mountains and sea stop them), normalised so the total is conserved. The heatmap, the per-tile label, region totals and the residence layer all read that same field, so a tinted tile always has people on it, a city is surrounded by its outskirts instead of a ring of zeros, and the heatmap scales logarithmically to the densest tile in the world, so a hamlet reads violet, a village core red, a town core orange and a city core yellow.
- **Population heatmap palette.** The density overlay now runs violet → magenta → red → orange → yellow instead of green → red, so it stands out from the terrain instead of dissolving into it (and never reads as water).
- **Map overlays under one menu.** Every Regions and Societies overlay now lives under a single map-mode button instead of crowding the top level.
- **Performance.** Every region's demographics are precomputed once at load, so opening a demographic overlay is instant; the aggregation now culls each region's settlement pressure to its neighbourhood, keeping worldgen linear even at 100% coverage.
- **Odyssey multi-layer worlds.** Off-surface world objects — orbital habitats, space sites — are now classified and skipped cleanly instead of logging unrecognised-type warnings, so regions, demographics and placement stay correct with space factions on the orbit layer.
- **Now compatible with Layered Atmosphere and Orbit.** The region-border and capital-marker map overlays are global draw layers with no planet layer of their own; LAO's world-render patch dereferenced that and null-referenced, black-worlding the map. The overlays are now pinned to the surface layer, so LAO renders cleanly — and it is no longer marked incompatible.
- **Fixed (with thanks to community member 96localk).** Every generated faction was created *hidden* — absent from the Factions tab, with no goodwill or leader — and unselected factions could leak into worlds. Factions now generate visible with leaders, only the factions you chose appear, and a one-time repair fixes worlds saved under the bug.
