...
- **Fixed: game froze when opening the Deserter network from a comms console with Vanilla Factions Expanded - Deserters (#37).** Opening the network fills a list of service quests, and the site-based quest scripts search a band of tiles around your colony using RimWorld's own "is this a valid settlement tile" check as their filter. Regions and Societies answered that check with a full placement evaluation for every candidate tile (thousands per search) and refused most of them on the player's supply rule, which stalled the dialog for a very long time and could starve quests of sites. The placement rules now apply only to checks a player is about to read (the settle button and the starting-site page), never to tile searches. Vanilla quest-site searches were affected the same way and are fixed by the same change.
