...
- **Now compatible with Map Preview (#45).** Map Preview generates the selected tile's map on a worker thread, and part of that flood-fills the same shared world flood filler our placement evaluation uses; the two collided and RimWorld logged `Nested FloodFill calls are not allowed` on every selection. The pane's placement line now waits while a preview is generating (detected through Map Preview's public API, no dependency added) and appears when it is done.
