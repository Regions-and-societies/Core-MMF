...
namespace RegionsAndSocieties
{
    ...
    public class FactionPlacementSettings : ModSettings
    {
        ...
        {
            if (def == null) return null;
            FactionPlacementProfile profile;
            if (!FactionPlacementDefaults.TryGet(def.defName, out profile))
            {
                bool hostile = def.hostileToFactionlessHumanlikes || def.permanentEnemy;
                profile = FactionPlacementDefaults.TechLevelDefault(def.defName, (int)def.techLevel, hostile);
            }
            // #46/#47/#63: kin, cluster and share defaults come from the faction's kind, not from the
            // registration — a registered profile that leaves them unset gets the same defaults a guessed
            // one does; one that sets them keeps its values.
            if (profile.clusterSize < 0) profile.clusterSize = DefaultClusterSize(def);
            if (profile.numberOfClusters < 0) profile.numberOfClusters = DefaultClusterCount(def);
            if (profile.placementShare <= 0f) profile.placementShare = Placement.PlacementShareRules.MigrateRangeToShareWeight(profile.baseCountRange.min, profile.baseCountRange.max);
            return profile;
        }
    }
}
